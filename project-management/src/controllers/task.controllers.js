...
const updateSubTask = asyncHandler(async (req, res) => {
    ...
    const subtask = await Subtask.findByIdAndUpdate(subTaskId, { $set: { title, isCompleted } }, { new: true });
    if (!subtask) throw new ApiError(404, "Subtask not found");
    const task = await Task.findById(subtask.task).select("project title");
    logActivity({ projectId: task?.project, actorId: req.user._id, action: ActivityActionEnum.SUBTASK_UPDATED,
        entityType: "subtask", entityId: subtask._id, entityName: subtask.title,
        metadata: { parentTaskTitle: task?.title, ...(isCompleted !== undefined && { isCompleted }) },
    }).catch(() => {});
...
const deleteSubTask = asyncHandler(async (req, res) => {
    ...
    if (!subtask) throw new ApiError(404, "Subtask not found");
    const task = await Task.findById(subtask.task).select("project title");
    logActivity({ projectId: task?.project, actorId: req.user._id, action: ActivityActionEnum.SUBTASK_DELETED,
        entityType: "subtask", entityId: subtask._id, entityName: subtask.title,
